...
{hotelsTable->nHotels = 0;}

//1.B
void hotelsTableSelect(tHotelTable hotelsTable, const char* city, float price, float distance, int points, tHotelTable *selectedHotelsTable)
{
	...
	for(i=0;i < hotelsTable.nHotels;i++)
	{
		...
		{
			if(hotelComputePoints(hotelsTable.hotels[i],price,distance) >= points)
			{
				hotelCopy(&selectedHotelsTable->hotels[selectedHotelsTable->nHotels],hotelsTable.hotels[i]);
...
float hotelsComputeAverageOccupation(tHotelTable hotelsTable, const char* city)
{
	...
	for(int k=0;k<hotelsTable.nHotels;k++)
	{
		if(strcmp(hotelsTable.hotels[k].city, city) == 0)
		{
			bookedRooms = (bookedRooms + ( ((float)hotelsTable.hotels[k].numRooms)*(hotelsTable.hotels[k].percentOccupation) /100.0f));
			freeRooms = (freeRooms + (hotelsTable.hotels[k].numRooms));
		}
	}
	...
	if (freeRooms == 0)
		...
		return ((100.0f * bookedRooms)/freeRooms);
}
