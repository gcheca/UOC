...
void readCustomer(tCustomer *customer, tError *retVal) 
{
	*retVal = OK;
	...
	printf("Place of Birth (%d char max, no spaces):\n>> ", MAX_CITY);
    scanf("%s",buffer);
	length = strlen(buffer);
	if (length<MAX_CITY) 
	{
	...
	printf("Client Profile (1 digit being 0=FAMILIAR, 1=CONSERVATIVE, 2=CONSERVATIVE, 3=LONELY, 4=CURIOUS, 5=INNOVATIVE, 6=DARED):\n>> ");
	scanf("%d",&number);
                                ...
                                char *city, float price, float distance, int points) 
{
	...
	for(i=0;i<appData.customers.nCustomers;i++)
	{
		customerRecommendation(appData.customers.table[i],appData.hotels,city,price,distance,points,result);
		printCustomer(appData.customers.table[i],i);
	}
